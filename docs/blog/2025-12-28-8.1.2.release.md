---
slug: 8.1.2-release
title: "8.1.2 - External Payments Program & Auto Connection"
authors: [hyochan]
tags: [release, android, ios, billing, external-payments]
---

v8.1.2 adds support for Google Play Billing Library 8.3.0's External Payments program, enableBillingProgramAndroid in InitConnectionConfig, and auto connection management for iOS.
...
### Auto Connection Management (iOS)

All API methods now automatically call `initConnection()` internally if the connection hasn't been established yet. This eliminates the need to manually call `initConnection()` before using any API on iOS.
...
> **Note**: While explicit `initConnection()` is no longer required on iOS, you may still want to call it during app startup to pre-initialize the StoreKit connection for faster subsequent API calls.
