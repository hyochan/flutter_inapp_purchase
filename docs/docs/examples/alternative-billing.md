...
class AlternativeBillingScreen extends StatefulWidget {
  @override
  _AlternativeBillingScreenState createState() =>
...
class _AlternativeBillingScreenState extends State<AlternativeBillingScreen> {
  ...
  Future<void> _initConnection() async {
    ...
    if (Platform.isAndroid) {
      _userChoiceSubscription = FlutterInappPurchase.userChoiceBillingAndroid
  ...
  void _handleUserChoiceBilling(UserChoiceBillingDetails details) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Alternative billing selected. Process payment now.'),
      ),
    );
  }
  ...
  Future<void> _handleIOSPurchase(Product product) async {
    try {
      await FlutterInappPurchase.instance.requestPurchase(
        RequestPurchaseProps.inApp(
          request: RequestPurchasePropsByPlatforms(
            ios: RequestPurchaseIosProps(sku: product.id, quantity: 1),
          ),
          useAlternativeBilling: true,
        ),
      );
          ...
          : SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (Platform.isAndroid) ...[
                    DropdownButton<AlternativeBillingModeAndroid>(
                      value: _billingMode,
                      isExpanded: true,
                      items: [
                        DropdownMenuItem(
                          value: AlternativeBillingModeAndroid.AlternativeOnly,
                          child: Text('Alternative Billing Only'),
                        ),
                        DropdownMenuItem(
                          value: AlternativeBillingModeAndroid.UserChoice,
                          child: Text('User Choice Billing'),
                        ),
                      ],
                      onChanged: (mode) {
                        if (mode != null) {
                  ...
                  if (_selectedProduct != null)
                    ElevatedButton(
                      onPressed: () => _handlePurchase(_selectedProduct!),
                      child: Text(
                        Platform.isIOS
                            ? 'Buy (External URL)'
                            : _billingMode ==
